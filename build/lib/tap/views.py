...
class Management(object):
    ...
    @view_config(route_name="permission_home", permission="view")
    def permission_home(self):
        with transaction.manager:
            users = DBSession.query(TapUser)
            context = dict(pagename=u"权限管理", users=users)
            context.update(common_vars(self.request))
            return render_to_response("templates/permission_home.html",
    ...
    def api_test(self):
        ...
        try:
            result = Program(config).run(paras)
            result_json = json.dumps(result, cls=CadaEncoder)
